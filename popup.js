...
// Định nghĩa hàm dự đoán tính độc hại của URL
function predict(url) {
    // Phân tích các đặc điểm của URL
    const urlFeatures = {
    ...
    // Dự đoán tính độc hại của URL
    const result = net.run(urlFeatures);
    console.log(result)
    // Trả về kết quả
    if (result.status < 0.5) {
        return "Đây là link an toàn ( không bị phishing )" + "\n" + "Tỉ lệ dự đoán: "+ ((1 - result.status) * 100).toFixed(2) + "%" ;
    } else {
        return "Đây là link có thể bị phishing!" + "\n" + "Tỉ lệ dự đoán: " + (result.status * 100).toFixed(2) + "%";
    }
}
